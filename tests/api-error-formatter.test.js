...
test('formatApiError maps HTTP 401 to auth failure', () => {
  ...
  assert.equal(result.message, '认证失败，请检查 API Key 或网关鉴权 Header');
...
test('formatApiError maps HTTP 404 to invalid URL without API body', () => {
  ...
  assert.equal(result.message, '请求地址无效，请检查 API 地址和请求路径');
  assert.equal(result.action, 'settings');
});
...
test('formatApiError maps HTTP 404 model errors to model configuration', () => {
  ...
  assert.equal(result.message, '模型不存在或无权限，请检查模型名称和账号权限：Model deepseek-chat not found');
  assert.equal(result.action, 'settings');
});
...
test('formatApiError uses options.statusCode with API error body', () => {
  ...
  assert.equal(result.message, '认证失败，请检查 API Key 或网关鉴权 Header：Incorrect API key provided');
  assert.equal(result.action, 'settings');
});
...
test('formatApiError uses options.statusCode with invalid JSON body', () => {
  ...
  assert.equal(result.message, '服务器返回内容异常，请稍后重试');
  assert.equal(result.action, 'wait');
});
...
test('formatApiError maps ETIMEDOUT', () => {
  ...
  assert.equal(result.message, '连接超时，请检查网络或稍后重试');
  assert.equal(result.action, 'retry');
});
...
test('detectGarbledContent returns false for normal text', () => {
  assert.equal(detectGarbledContent('这是一段正常的中文文本'), false);
...
test('detectGarbledContent detects mojibake replacement characters', () => {
  assert.equal(detectGarbledContent('正常文本閿燂拷閿燂拷閿燂拷异常'), true);
});

test('detectGarbledContent detects BOM residue', () => {
  assert.equal(detectGarbledContent('閿橈拷text'), true);
  assert.equal(detectGarbledContent('茂禄驴text'), true);
});

test('detectGarbledContent detects garbled pattern', () => {
  assert.equal(detectGarbledContent('闁挎绗滅缓异常'), true);
  assert.equal(detectGarbledContent('闁挎绗滅缓'), true);
});
